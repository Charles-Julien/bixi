...
df_main <- df_main %>% mutate(
              ...
              date_ = as.Date(paste(year,mm,dd,sep="/")),
              
              week_num = as.numeric(strftime(date, format = "%V")),
 
              hot_weather_ind = factor(ifelse(temp > 15, 'Hot', 'Cold')),# create a new variable indicating whether the average daily trip duration exceeds 15 minutes, and explore models for this new variable.
...
Linear interpolation emerged as a valuable technique to impute missing
revenue data for members. This method assumes a linear relationship
between observed values on either side of the missing data points. When
encountering missing revenue values for members, linear interpolation
estimates these values by considering the known revenues immediately
preceding and following the missing positions. Utilizing the equation of
a straight line, the imputation process calculates the missing revenue
value based on the linear trend between the available data points.

The imputeTS package in R provides a convenient function,
na.interpolation, with the option set to "linear," making it
straightforward to implement linear interpolation for imputing missing
revenue data in this specific scenario. However, it is crucial to note
that linear interpolation presupposes a linear trend in the data, and
its suitability depends on the underlying characteristics of the revenue
time series.
...
on the revenue (`rev_pred`) generated by trips leaving from a specified
station.

## Model Linear regression

```{r Seasonal_effect_revenue, echo=FALSE}
seasonal_effect_rev_model <- lm(rev_imputed ~ mm + temp + rain + mem, data = df_main)
...
```{r}
seasonal_effect_rev.ar <- gls(rev_imputed ~ temp + rain + mm, correlation = corAR1(form = ~ 1 | station), data = df_main)
...
```{r}
#df_main
seasonal_effect_rev.rand_int <- lme(rev_imputed ~ temp + rain + mm, random = ~ 1 | station, data = df_main)
...
seasonal_effect_rev.rand_coef <- lme(rev_imputed ~ temp + rain + mm, random = ~1 + temp |
...
at the borderline of significance (p-value = 0.0745). DO A GLOBAL F TEST
...
NOT COMPLETE \### Normality of residuals
