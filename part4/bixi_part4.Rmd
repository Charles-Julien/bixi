...
```{r}
seasonal_effect_rev_gls <- gls(rev_imputed ~ temp + rain + mm, correlation = corAR1(form = ~ 1 | station), data = df_main)
...
```{r}
getVarCov(seasonal_effect_rev_gls,individual = 1)
```

```{r}
#df_main
seasonal_effect_rev_gls <- lme(rev_imputed ~ temp + rain + mm, random = - 1 | df_main$station, data = df_main)
