...
df_main <- df_main %>% mutate(
              ...
              #Overwrite old wday? CHIKE: it was to make it an ordinal categorical variable rather than nominal
...
- **wday (Weekday)**: Compared to the reference weekday (likely Friday since it's not listed):
...
time_pattern_dur_model <- lm(dur ~ dd + wday + holiday + n_AM + n_PM +n_AM*n_PM, data=df_main) # Should we look at the interaction term here? duration is correlated to the number of trips which explains the favourable f test. Data leakage?
...
```{r}
model <- lm(n_AM_PM_delta ~ wknd_ind + season + rain_ind + mem, data = df_main) # Goal is to look at stations more used in the mornings than afternoons
summary(model)
```
```{r}
df_main # SHORTER TRIPS ON WEEKDAYS THAN WEEKENDS
model <- lm(avg ~ wknd_ind + season + rain_ind + mem, data = df_main)
summary(model)
```
...
model <- lm(avg ~ (rain_ind *mem) + wknd_ind, data = df_main)
summary(model)
```
```{r}
#HIGHER REVENUE PER TRIP IN SPRING AND SUMMER THAN WINTER
model <- lm(rev_per_trip ~ wknd_ind + season + rain_ind, data = df_main)
summary(model)
```
```{r}
df_main # AS BIXI SEASON GOES ON, WEEKDAY NUMBER OF TRIPS GO UP WITH A STATISTICAL SIGNIFICANCE
model <- lm(n_tot ~ (wknd_ind*week_num), data = df_main)
summary(model)
```
## residual Analysis
```{r echo=FALSE}
